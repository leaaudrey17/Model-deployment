# -*- coding: utf-8 -*-
"""Untitled0.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1KqmNAMPDmpIR5VcMLbdLLBilXWNJcWSw
"""

# -*- coding: utf-8 -*-
import streamlit as st
import pickle
import numpy as np
import joblib  # Ensure joblib is installed: pip install joblib

# Load model dari file pickle
model_path = "iris_model.pkl"  # Ganti path jika diperlukan
try:
    with open(model_path, "rb") as model_file:
        model = pickle.load(model_file)
except FileNotFoundError:
    st.error(f"Model file '{model_path}' tidak ditemukan. Pastikan file tersedia.")

# Judul aplikasi
st.title("Iris Flower Prediction App")
st.write("Masukkan ukuran bunga untuk mendapatkan prediksi spesies.")

# Input fitur dari pengguna
sepal_length = st.number_input("Sepal Length (cm)", min_value=4.0, max_value=8.0, step=0.1)
sepal_width = st.number_input("Sepal Width (cm)", min_value=2.0, max_value=5.0, step=0.1)
petal_length = st.number_input("Petal Length (cm)", min_value=1.0, max_value=7.0, step=0.1)
petal_width = st.number_input("Petal Width (cm)", min_value=0.1, max_value=2.5, step=0.1)

# Tombol prediksi
if st.button("Predict"):
    if 'model' in locals():
        # Membuat array input untuk model
        input_features = np.array([[sepal_length, sepal_width, petal_length, petal_width]])

        # Prediksi spesies
        prediction = model.predict(input_features)
        species_dict = {0: "Setosa", 1: "Versicolor", 2: "Virginica"}
        predicted_species = species_dict.get(prediction[0], "Unknown")

        # Tampilkan hasil prediksi
        st.success(f"Model memprediksi bunga ini adalah **{predicted_species}**")
    else:
        st.error("Model belum dimuat. Periksa file model Anda.")

pip install streamlit joblib numpy